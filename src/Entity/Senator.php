...
/**
 * @ORM\Entity(repositoryClass=SenatorRepository::class)
 */
class Senator
{
    /**
    ...
    /**
     * @ORM\OneToOne(targetEntity=ExtraData::class, mappedBy="senator", cascade={"persist", "remove"})
     */
    private $extraData;

    /**
     * @ORM\OneToMany(targetEntity=OtherNote::class, mappedBy="senator", orphanRemoval=true)
     */
    private $notes;

    /**
     * @ORM\OneToMany(targetEntity=BoardMinute::class, mappedBy="senator", orphanRemoval=true)
     */
    ...
    public function setExtraData(?ExtraData $extraData): self
    {
        ...
        $newSenator = null === $extraData ? null : $this;
        if ($extraData->getSenator() !== $newSenator) {
            $extraData->setSenator($newSenator);
        }
    ...
    public function removeNote(OtherNote $otherNote): self
    {
        if ($this->notes->contains($otherNote)) {
            ...
            if ($otherNote->getSenator() === $this) {
                $otherNote->setSenator(null);
            }
        }
    ...
    public function removeBoardMinute(BoardMinute $boardMinute): self
    {
        if ($this->boardMinutes->contains($boardMinute)) {
            ...
            // set the owning side to null (unless already changed)
            if ($boardMinute->getSenator() === $this) {
                $boardMinute->setSenator(null);
            }
        }
