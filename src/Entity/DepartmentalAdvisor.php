...
/**
 * @ORM\Entity(repositoryClass=DepartmentalAdvisorRepository::class)
 */
class DepartmentalAdvisor
{
    /**
     * @ORM\Id
     * @ORM\GeneratedValue()
    ...
    /**
     * @ORM\OneToOne(targetEntity=ExtraData::class, mappedBy="departmentalAdvisor", cascade={"persist", "remove"})
     */
    private $extraData;

    /**
     * @ORM\OneToMany(targetEntity=MPDPRNote::class, mappedBy="departmentalPresident", cascade={"remove"}, orphanRemoval=true)
     */
    private $notes;

    /**
     * @ORM\OneToMany(targetEntity=OtherNote::class, mappedBy="departmentalAdvisor", cascade={"remove"}, orphanRemoval=true)
     */
    private $otherNotes;

    /**
     * @ORM\OneToMany(targetEntity=BoardMinute::class, mappedBy="departmentalAdvisor", cascade={"remove"}, orphanRemoval=true)
     */
    ...
    public function getCantonCode(): ?string
    {
        return $this->cantonCode;
    }

    public function setCantonCode(?string $cantonCode): self
    {
    ...
    public function getCantonLabel(): ?string
    {
        return $this->cantonLabel;
    }

    public function setCantonLabel(?string $cantonLabel): self
    {
        $this->cantonLabel = $cantonLabel;
    ...
    public function setExtraData(?ExtraData $extraData): self
    {
        ...
        $newDepartmentalAdvisor = null === $extraData ? null : $this;
        if ($extraData->getDepartmentalAdvisor() !== $newDepartmentalAdvisor) {
            $extraData->setDepartmentalAdvisor($newDepartmentalAdvisor);
        }
    ...
    public function addNote(MPDPRNote $note): self
    {
        if (!$this->notes->contains($note)) {
            $this->notes[] = $note;
            $note->setDepartmentalPresident($this);
        }
    ...
    public function removeNote(MPDPRNote $note): self
    {
        if ($this->notes->contains($note)) {
            ...
            if ($note->getDepartmentalPresident() === $this) {
                $note->setDepartmentalPresident(null);
            }
        }
    ...
    public function addOtherNote(OtherNote $otherNote): self
    {
        if (!$this->otherNotes->contains($otherNote)) {
            ...
            $otherNote->setDepartmentalAdvisor($this);
        }
    ...
    public function removeOtherNote(OtherNote $otherNote): self
    {
        if ($this->otherNotes->contains($otherNote)) {
            ...
            // set the owning side to null (unless already changed)
            if ($otherNote->getDepartmentalAdvisor() === $this) {
                $otherNote->setDepartmentalAdvisor(null);
            }
        }
    ...
    public function addBoardMinute(BoardMinute $boardMinute): self
    {
        if (!$this->boardMinutes->contains($boardMinute)) {
            ...
            $boardMinute->setDepartmentalAdvisor($this);
        }
    ...
    public function removeBoardMinute(BoardMinute $boardMinute): self
    {
        if ($this->boardMinutes->contains($boardMinute)) {
            ...
            // set the owning side to null (unless already changed)
            if ($boardMinute->getDepartmentalAdvisor() === $this) {
                $boardMinute->setDepartmentalAdvisor(null);
            }
        }
