...
/**
 * @ORM\Entity(repositoryClass=MunicipalAdvisorRepository::class)
 */
class MunicipalAdvisor
{
    /**
     * @ORM\Id
     * @ORM\GeneratedValue
    ...
    /**
     * @ORM\OneToOne(targetEntity=ExtraData::class, mappedBy="municipalAdvisor", cascade={"persist", "remove"})
     * 
     */
    private $extraData;

    /**
     * @ORM\OneToMany(targetEntity=OtherNote::class, mappedBy="municipalAdvisor", orphanRemoval=true)
     */
    private $notes;

    /**
     * @ORM\OneToMany(targetEntity=BoardMinute::class, mappedBy="municipalAdvisor", orphanRemoval=true)
     */
    ...
    public function setExtraData(?ExtraData $extraData): self
    {
        ...
        $newMunicipalAdvisor = null === $extraData ? null : $this;
        if ($extraData->getMunicipalAdvisor() !== $newMunicipalAdvisor) {
            $extraData->setMunicipalAdvisor($newMunicipalAdvisor);
        }
    ...
    public function addNote(OtherNote $otherNote): self
    {
        if (!$this->notes->contains($otherNote)) {
            $this->notes[] = $otherNote;
            $otherNote->setMunicipalAdvisor($this);
        }

        return $this;
    }

    public function removeNote(OtherNote $otherNote): self
    {
        if ($this->notes->contains($otherNote)) {
            $this->notes->removeElement($otherNote);
            // set the owning side to null (unless already changed)
            if ($otherNote->getMunicipalAdvisor() === $this) {
                $otherNote->setMunicipalAdvisor(null);
            }
        }
    ...
    public function addBoardMinute(BoardMinute $boardMinute): self
    {
        if (!$this->boardMinutes->contains($boardMinute)) {
            ...
            $boardMinute->setMunicipalAdvisor($this);
        }
    ...
    public function removeBoardMinute(BoardMinute $boardMinute): self
    {
        if ($this->boardMinutes->contains($boardMinute)) {
            ...
            // set the owning side to null (unless already changed)
            if ($boardMinute->getMunicipalAdvisor() === $this) {
                $boardMinute->setMunicipalAdvisor(null);
            }
        }
